...
def build_huffman_tree(frequencies):
    ...
    while len(heap) > 1:
        ...
        print(Fore.YELLOW + "Merging nodes:", f"{left.char} ({left.freq})",
              "and", f"{right.char} ({right.freq})")
        time.sleep(0.5)
...
def generate_codes(node, prefix="", codebook=None):
    ...
    if node:
        if node.char is not None:
            ...
            print(Fore.GREEN + f"Assigning code to character {node.char!r}: {codebook[node.char]}")
            time.sleep(0.3)
